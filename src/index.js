...
function startRecursive() {
    ...
    function doPromise(arr) {
        ...
        arr.forEach(item => {
            PromiseArr.push(processUser(item.id))
        })

        let handleAllPromises = Promise.all(PromiseArr);
        return handleAllPromises.then( values => {
            const val = values.reduce((sum, current) => {
                return sum + current;
            });
            return total += val;
        });
    ...
    async function recurse(x) {
        if (x === 0) {
            let arr = users.slice(x, x + 10);
            return await doPromise(arr, total);
        } else {
            let arr = users.slice(x - 10, x);
            await doPromise(arr, total)
            return recurse(x - 10, x);
        }
    }
    return new Promise((resolve, reject) => {
        resolve(recurse(100));
